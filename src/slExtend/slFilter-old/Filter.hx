...
class Filter
{
	...
	{
		// this filtering is chaining all the functions stored in filterRecord
		/*for(filterRecord in filters)
		{
			value = filterRecord.filter(value, context);
		}*/
		
		//this filtering is launching only the function with the most priority stored in filterRecord
		value = filters[filters.length].filter(value, context);
	 ...
	 */
	public function addFilter(filter : FilterFunction, priority : Int) : Void
	{
		...
		trace(filters);
		trace(filters[0].filter);
		trace(Reflect.fields(filters[0].filter));
		//Reflect.callMethod(filters[0].filter,
	}
	 ...
	 */
	public function removeFilter(filter : FilterFunction) : Void
	{
		for(filterRecord in filters)
		{
 ...
 */
typedef FilterRecord = { filter: FilterFunction, priority: Int };
